...
class LocalDbCore(_LocalDbCoreBase):
    ...
    def add_synced(cls, table, server, id_name, id_value, names, obj=None, values=None):
        ...
        if row:
            ...
            _ = cursor.execute("""INSERT INTO {table} ({names}) VALUES ({values})""".format(
                table = table,
                names = ', '.join(_names),
    ...
    @classmethod
    @_LocalDbCoreBase.into_db_executor
    def get_synced(cls, table, server, id_name, id_value, names, defaults, need_set_id=True):
        conn = cls.get_instance().conn
        cursor = conn.cursor()
        names_str = ', '.join(names)
        cursor.execute('SELECT '+names_str+' FROM '+table+' WHERE server=? AND '+id_name+'=?', (server, id_value))
        point = None
        for row in cursor:
            point = DbPoint()
            for i in range(len(names)):
                setattr(point, names[i], row[i])
            break
        if not point:
            point = DbPoint()
            for i in range(len(names)):
                setattr(point, names[i], defaults[i])
        if need_set_id:
            setattr(point, id_name, id_value)
        return point
