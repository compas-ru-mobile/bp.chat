...
from .drawers import (MessageDrawer, WordsLineDrawer, FileLineDrawer, QuoteAuthorDrawer, QuoteLineDrawer, QuoteFileLineDrawer, WORD_TYPE_LINK,
                      LINE_TYPE_FILE)
...
class MessagesListView(ListView):
    ...
    def mousePressEvent(self, e):
        delegate = self.itemDelegate()
        self.mouse_start_dt = datetime.now()
    ...
    def mouseReleaseEvent(self, e):
        ...
        if self.mouse_start_dt and (datetime.now() - self.mouse_start_dt).total_seconds() <= 0.3:
        ...
        if e.button() == Qt.LeftButton:
            ...
            if item and type(item.item) != LoadMessagesButton:
                ...
                if drawer:
                    ...
                    quote = item.message.quote
                    if quote:
                        lines = drawer.quote_lines + lines
                    ...
                    for line in lines:
                        for w in line:
                            if hasattr(w, 'rect'):
                                w_rect = w.rect
                                if w_rect[0] <= x < w_rect[2] and w_rect[1] <= y < w_rect[3]:# + w_rect[3]: // + w_rect[2]
                                    _found = True
                                    self._current_selection = []

                                    self._custom_selection.active = False
                                    self._custom_selection.start = (w_rect[0]-1, w_rect[1] + rect.top() - 1)
                                    self._custom_selection.end = (w_rect[2]+1, w_rect[1] + rect.top() + 3) # + w_rect[2] w_rect[1] +

                                    self.delegate.on_custom_selection_changed(self._custom_selection)
...
class ListDelegate(QItemDelegate):
    ...
    def prepareMessageFont(self):
        font = QFont("Arial")
        font_pixel_size = self.message_font_size()
    ...
    def prepareTimeFont(self):
        font = QFont("Arial")
        font_pixel_size = self.time_font_size()
        if font_pixel_size < 3:
            font_pixel_size = 3
        font.setPixelSize(font_pixel_size)
        font.setBold(False)
    ...
    def on_mouse_pos_changed(self, pos, will_model_reseted=False):
        ...
        if item and type(item.item) != LoadMessagesButton:
            ...
            if self.calc_and_change_is_on_pos('_mouse_on_name', pos, item,
                                              QRect(QPoint(int(_left-10), int(title_top) #rect.top() + 16
                                                           ),
                                                    QPoint(int(_right), int(title_top + 14) #rect.top() + 16 + 14
                                                           ))):
                changed = True
            ...
            if drawer:
                ...
                if self._mouse_on_link:
                    for name in ('image', 'name'):
                        self.change_value('_mouse_on_'+name, None)
...
class ListModel(QAbstractListModel):
    ...
    def reset_model(self, action='_reset_model_do_action', change_need_to_bottom=True):
        ...
        if self._need_reset_timer:
            return
        self._need_reset_timer = Timer(0.25, self._reset_model)
...
class MessagesListDelegate(ListDelegate):
    ...
    _PARTS = PMessageLayout(
        ...
        PVLayout(
            PMessageLogin(),
            PMessage(margin_top=5, debug=P_DEBUG, padding_left=10, padding_top=10),
    ...
    def prepareMessageText(self, item, second_text, left_top_right_bottom):
        ...
        if quote:
            _quote_author = QuoteAuthorDrawer(quote, drawer, top=top_now)
            ...
            for i, line in enumerate(drawer.quote_lines):
                to_new_qoute_lines, top_now = drawer.prepare_line(line, (left+8, top_now, right), space_width, line_height,
                                                                lambda a, top: QuoteLineDrawer(a, quote, drawer, top=top))
            ...
            if drawer.quote_file:
                top_now -= 0.8*line_height
                drawer.quote_file._top = top_now
                _add += drawer.quote_file.line_height
                top_now += drawer.quote_file.line_height
        ...
        if len(new_lines_without_files) > 0:
            message_height += 10

        if drawer.file_line:
            file_line = drawer.file_line
            file_line._top = top_now
    ...
    def drawMessageText(self, painter, line_height_and_lines, left_top_right_bottom, item):
        ...
        if hasattr(lines, 'draw'):
            ...
            _last_splitted_line, _last_splitted_i = None, -1
            i = -1
            for _, words_line in enumerate(lines):
                i += 1

                line_height = words_line.line_height
                try:
                    top_now = words_line.top # NEW !!!
                ...
                if selected_line != None:
                    ...
                    if words_line.IsSplittedLine:
                        if _last_splitted_line:
                            i -= 1
                            _last_splitted_line = _last_splitted_line + ' ' + selected_line
                        else:
                            _last_splitted_line = selected_line
                        _last_splitted_i = i
                    else:
                        if _last_splitted_line:
                            i -= 1
                            selected_line = _last_splitted_line + ' ' + selected_line
                            _last_splitted_line = None
            ...
            if _last_splitted_line:
                selected_lines.append(_last_splitted_line)
                last_selected_line_i = _last_splitted_i
    ...
    def draw_right_text(self, painter, message, left_top_right_bottom):
        ...
        _top_left = QPoint(
            int(right - (date_text_width + padding_x + 5 + QFontMetrics(font).height())), #+ self.listView.width_add,
            int(bottom - font_h - padding_y)
        )
    ...
    def selected_color(self, selected_items):
        if len(selected_items) == 1:
            return
        return color_from_hex('#e7b362')#'#2C8DF3')#'#fae298')
...
class MessagesListModel(ListModel):
    ...
    def on_need_download_20(self, min_message_id, need_check_last_dt = False):
        pass
    ...
    def getItemHeight(self, item, option, width):
        ...
        if type(item.message) != LoadMessagesButton:
            ...
            if type(item.message) == MessageImitate:
                return 70
    ...
    def set_items_dict(self, val):
        ...
        messages_dict = { int(k):v for k, v in items }

        need_last_20 = True
        #need_20_hole_id = None
        for _, m in messages_dict.items():
            if m.prev_id == 0:
                need_last_20 = False
                break
            # else:
            #     prev_id = int(m.prev_id)
            #     if need_20_hole_id == None and prev_id != 0 and prev_id not in messages_dict:
            #         need_20_hole_id = m_id
            # if not need_last_20 and need_20_hole_id != None:
            #     break

        messages_dict = { k:v for k, v in messages_dict.items() if self.filt(k, v) }

        #messages_dict = { int(k):v for k, v in items if self.filt(k, v) }
        keys_list = sorted(list(messages_dict.keys()), key=lambda key: (messages_dict[key].datetime, key))

        if need_last_20: #len(messages_dict) >= 20 or (self.is_only_files or self.is_only_favorites) or min_message_id == None: # FIXME
            messages_dict.keys()
...
class MessageImitate:
    pass

class MessageImitateItem(ListModelItem):

    message = MessageImitate()
