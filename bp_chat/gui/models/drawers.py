...
from bp_chat.logic.chat_api.ChatApiCommon import SMILES, SMILES_DICT

from ..core.draw import icon_from_file, draw_icon_from_file
...
class MessageDrawer:
    ...
    def __init__(self, message, font, text, delegate):
        ...
        if hasattr(message.message, 'quote') and message.message.quote:
            ...
            if quote_file and quote_file not in (0, '0'):
                self.quote_file = QuoteFileLineDrawer(quote_file, quote_filename, quote_file_size, self)
        ...
        if hasattr(message.message, 'file'):
            ...
            if has_file:
                ...
                self.file_line = FileLineDrawer(message.message.getFile(), filename, message.message.getFileSize(), self)
    ...
    def split_line(self, line):
        ...
        for i, w in enumerate(_words):
            ...
            if link:
                w_drawer = LinkWordDrawer(w, self, link, last=_last_word_drawer)
            else:
                w_drawer = WordDrawer(w, self, last=_last_word_drawer)
            _last_word_drawer = w_drawer
    ...
    def prepare_line(self, line, left_top_right, space_width, line_height, line_cls=None):
        ...
        if type(_words) == list:
            _words = line_cls(_words, top=top_now)
        _words._top = top_now
        ...
        for i, w_drawer in enumerate(_words):
            ...
            _added_line = False
            if r_right > right:
                li = line_cls(_words[last_i:i], top=top_now)
                li.IsSplittedLine = True
                to_new_lines.append(li)
                last_i = i
        ...
        if len(to_new_lines) == 0:
            to_new_lines.append(line_cls(_words, top=top_now))#-line_height))
            top_now += line_height
        elif last_i <= i:
            to_new_lines.append(line_cls(_words[last_i:], top=top_now))#-line_height))
            top_now += line_height
...
class LineBaseDrawer:
    line_type = LINE_TYPE_BASE
    first_word_left = 0
    IsSplittedLine = False
...
class WordsLineDrawer(LineBaseDrawer, list):

    _line_height = 14
    font_height = 14

    def __init__(self, lst, line_height=14, top=None):
    ...
    def draw_line(self, mes_drawer, painter, left_top_right_bottom, sel_start, sel_end):
        ...
        for w in self:
            ...
            for a, a_w in zip(w, w.widths):
                a_right = a_left + a_w
                a_top = top_now - self.font_height #+2

                if a in SMILES:
                    ww.append((a, a_left, a_top))
                else:
                    if len(ww) == 0 or type(ww[-1]) == tuple:
                        ww.append(["", a_left, a_top])
                    ww[-1][0] += a
            ...
            for s, alft, atop in ww:
                sm = SMILES_DICT.get(s, None)
                if sm:
                    draw_icon_from_file(painter, sm, alft, atop, w=20, h=20)
                else:
                    painter.drawText(int(alft), int(top_now), s)

            #painter.drawText(w_left, top_now, w)

            if need_draw_line_under:
                #painter.drawLine(x, y + 3, x_end, y + 3)
                painter.drawLine(int(w_left), int(top_now+2), int(a_right), int(top_now+2))
...
class FileLineDrawer(LineBaseDrawer):
    ...
    def __init__(self, file_uuid, filename, filesize, message_drawer, top=None):
    ...
    def draw_line(self, mes_drawer, painter, left_top_right_bottom, sel_start, sel_end):
        ...
        text_rect = QRect(QPoint(int(left+self.first_word_left), int(top_now-14)), QPoint(int(right), int(bottom_now-14)))
...
class QuoteDrawAdd:
    ...
    def draw_left_line(self, painter: QPainter, left, top, bottom, minus_line_height=None):
        ...
        painter.drawLine(int(left), int(top-lh+1), int(left), int(bottom-lh-1))

        painter.setPen(temp_pen)


class QuoteAuthorDrawer(LineBaseDrawer, QuoteDrawAdd):

    first_word_left = 10
    # line_height = 8
    margin_first_top = 4

    def __init__(self, quote: QuoteInfo, message_drawer, top=None):
        self.quote = quote
        self.message_drawer = message_drawer
        self._top = top
...
class QuoteLineDrawer(WordsLineDrawer, QuoteDrawAdd):

    first_word_left = 10
    _line_height = 14
    is_last_quote_line = False

    def __init__(self, lst, quote: QuoteInfo, message_drawer, top=None):
        self.quote = quote
        self.message_drawer = message_drawer
        super().__init__(lst, top=top)
...
class QuoteFileLineDrawer(FileLineDrawer, QuoteDrawAdd):
...
class WordDrawer(str):
    ...
    @staticmethod
    def __new__(cls, word: str, message_drawer: MessageDrawer, word_type: int = WORD_TYPE_SIMPLE, last = None):
        is_bold = False
        is_bold_fin = False
        if '<b>' in word: #.startswith('<b>'):
            word = word.replace('<b>', '') #word = word[3:]
            is_bold = True
        elif last and last.is_bold and not last.is_bold_fin:
            is_bold = True

        if '</b>' in word: #.endswith('</b>'):
            is_bold_fin = True
            word = word.replace('</b>', '')

        # for sm in SMILES:
        #     if sm in word:
        #         word = word.replace(sm, "?")

        obj = str.__new__(cls, word)
        obj.message_drawer = message_drawer
        obj.word_type = word_type
        obj.is_bold = is_bold
        obj.is_bold_fin = is_bold_fin
...
class LinkWordDrawer(WordDrawer):
    ...
    @staticmethod
    def __new__(cls, word: str, message_drawer, url: str, last = None):
        obj = WordDrawer.__new__(cls, word=word, message_drawer=message_drawer, word_type=WORD_TYPE_LINK, last=last)
        obj.url = url
        return obj
