...
class PLayout(PBase):
    ...
    def get_calced_sizes(self, item, delegate, size_tuple):
        ...
        for i, child in enumerate(self.childs):
            ...
            if len_i > 0:
                w, h = h, w
            ch_kwargs = child.kwargs
            if len_i == 0:
                strong_w += ch_kwargs.get('margin_left', 0) + ch_kwargs.get('margin_right', 0)
            else:
                strong_w += ch_kwargs.get('margin_top', 0) + ch_kwargs.get('margin_top', 0)
            if h is not None:
                calced_sizes_2[i] = h
            if w is not None:
                strong_w += w
...
class PMessage(PBase):
    ...
    def draw_line(self, painter, line, left, top, right, bottom, flags=Qt.AlignLeft | Qt.AlignTop):
        super().draw_line(painter, line, left+10, top+10, right, bottom, flags)
    ...
    def _draw(self, painter: QPainter, delegate, item, rect_tuple: tuple):
        ...
        if second_text != None:
            ...
            left_top_right_bottom = (left+5, top+12+5, right, bottom-5)
